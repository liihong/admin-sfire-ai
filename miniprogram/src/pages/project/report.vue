  ...
  <view class="report-page">
    ...
    <scroll-view class="report-scroll" scroll-y>
      <view class="report-wrapper">
        <!-- Loading状态 - 步骤式进度提示 -->
        <ReportLoading v-if="isLoadingReport" ref="reportLoadingRef" />
            ...
            <view class="report-section">
              ...
              <view class="section-content">
                <view class="persona-tags">
                  <view v-for="(tag, index) in reportData.report.persona_tags" :key="index" class="tag-item">
    ...
    <view class="page-footer">
      ...
      <view class="footer-btn save-btn" @tap="handleSave"
        :class="{ disabled: isSaving || !reportData || isLoadingReport }">
...
// 加载步骤组件引用
const reportLoadingRef = ref<InstanceType<typeof ReportLoading> | null>(null)
...
async function generateReport() {
  ...
  // 等待下一个tick确保组件已渲染，再初始化步骤并开始动画
  await new Promise(resolve => setTimeout(resolve, 50))
  reportLoadingRef.value?.initSteps()
  ...
  try {
    ...
    // 确保所有步骤都标记为完成
    reportLoadingRef.value?.completeAllSteps()

    // 报告生成成功，延迟一小段时间让用户看到完成状态
    ...
    // 生成报告失败
    const errorMsg = error instanceof Error ? error.message : '报告生成失败，请重试'
...
async function handleSave() {
  ...
  try {
    ...
    const projectFormData = ipCollectFormDataToProjectFormData(formData.value)
