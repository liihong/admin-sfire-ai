...
export interface LoginRequest {
  code: string
  /** 不传则服务端用默认 AppID；租户小程序应传当前实例 AppID（见 loginWithCode） */
...
export function loginWithCode(code: string) {
  const aid = getMiniProgramAppId()
  const payload: LoginRequest & { phone_code?: string; scene?: string } = { code }
  if (aid) payload.wechat_app_id = aid
...
export function loginWithPhoneForPc(data: { code: string; phone_code: string; scene: string }) {
  ...
  return request<{ success: boolean }>({
    url: '/api/v1/client/auth/login',
    method: 'POST',
    data: aid ? { ...data, wechat_app_id: aid } : data,
    needToken: false,
    showLoading: false
  })
}
