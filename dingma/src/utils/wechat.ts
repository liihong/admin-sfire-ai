...
/**
 * 当前运行环境的小程序 AppID（微信从 manifest 注入，用于后端 tenants.wechat_app_id 匹配）
 */
export function getMiniProgramAppId(): string | undefined {
  try {
...
function handleFallback(fallbackUrl?: string): void {
  ...
  if (pages.length > 1) {
    ...
    if (fallbackUrl.startsWith('/pages/')) {
      ...
      const tabBarPages = ['/pages/home/index', '/pages/mine/index']
