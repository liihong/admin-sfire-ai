...
**微信支付回调常用 IP 段**（按需补充）：
- 182.254.48.0/24、140.207.54.0/24、101.226.103.0/24：常用出口
- **121.51.58.0/24**：上海 CAP 出口（121.51.58.x 回调必须包含）

**注意**：
- 若 `WECHAT_PAY_IP_WHITELIST` 为空，则跳过 IP 验证（便于调试，生产环境建议配置）
- 微信支付可能使用多个 IP 段，若回调被拒绝可扩展白名单，参考[微信支付商户文档](https://pay.weixin.qq.com/doc/v2/merchant/4011984721)
...
**现象**：用户支付成功、已扣费，但算力未增加。

**原因**：微信支付是**异步回调**机制。用户支付成功后，微信会向 `WECHAT_PAY_NOTIFY_URL` 发起 POST 请求，后端收到回调并验证通过后才会充值算力。若回调失败，则算力不会到账。
...
- **有该日志**：说明微信已发起回调且到达了你的服务器，继续排查后续步骤
- **无该日志**：说明回调未到达，可能是网络、URL、防火墙等问题

#### 2. 若回调未到达，检查：

| 检查项 | 说明 |
|--------|------|
| **回调 URL** | `WECHAT_PAY_NOTIFY_URL` 必须为公网可访问的 HTTPS 地址，如 `https://your-domain.com/api/v1/client/coin/recharge/callback` |
| **防火墙/安全组** | 确保 443 端口对微信服务器 IP 开放 |
| **Nginx/代理** | 若使用反向代理，确保能正确转发 POST 请求到后端 |
| **微信商户平台** | 产品中心 → 开发配置 → 支付配置，确认回调 URL 与 `WECHAT_PAY_NOTIFY_URL` 一致 |

#### 3. 若回调到达但处理失败，查看日志中的具体错误：

| 日志关键词 | 可能原因 | 解决方案 |
|------------|----------|----------|
| `支付回调IP不在白名单` | 代理未正确传递 `X-Forwarded-For`，或微信 IP 段未覆盖 | 1. 确认 Nginx 配置了 `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for`<br>2. 扩展 `WECHAT_PAY_IP_WHITELIST`，参考微信官方文档的 IP 段 |
| `支付回调签名验证失败` | `WECHAT_PAY_API_KEY` 与商户平台不一致 | 校验 API 密钥（32 位，v2 API 密钥） |
| `支付金额不匹配` | 订单金额与回调金额不一致 | 检查订单创建与回调金额 |
| `订单不存在` | 订单号格式或数据库不一致 | 检查订单创建与存储逻辑 |

#### 4. 代理场景下的 IP 白名单

若使用 Nginx 等反向代理，`request.client.host` 可能是 `127.0.0.1`。代码会优先使用 `X-Forwarded-For` 获取真实 IP。若仍失败，需确认：

- Nginx 配置：`proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;`
- 多层代理时，`X-Forwarded-For` 可能包含多个 IP，代码取第一个（通常是客户端 IP）

#### 5. 小程序端改进（已实现）

支付成功后，小程序会轮询订单状态（约 60 秒）等待回调处理完成。若超时仍为 pending，会提示「支付成功，算力到账可能有延迟，请稍后刷新」。

#### 6. 手动补单（可选）

若确认用户已支付但回调失败，可联系管理员在后台根据订单号或微信交易号进行人工补单。
