...
class UserService:
    ...
    async def create_user(self, user_data: UserCreate) -> dict:
        ...
        if user_data.phone:
            ...
            if existing.scalar_one_or_none():
                raise BadRequestException(msg="手机号已被注册")
        
        # 转换等级
        level_enum = UserLevel(user_data.level) if user_data.level else UserLevel.NORMAL
    ...
    async def update_user(
        ...
        if "level" in update_data and update_data["level"]:
            update_data["level"] = UserLevel(update_data["level"])
    ...
    async def change_status(self, user_id: int, status: int) -> None:
        ...
        logger.info(f"User status changed: {user.username} -> {status_text}")
